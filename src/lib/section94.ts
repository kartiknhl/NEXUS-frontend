...
export async function buildSection94Notice(
  ...
  const finalEvidenceHash = evidenceHash ?? (await computeEvidenceHash({ trace, caseData }));
  ...
  doc.setFontSize(8);
  doc.setTextColor(70);
  doc.text("CERTIFICATE UNDER SECTION 63 BHARATIYA SAKSHYA ADHINIYAM (BSA), 2023", MARGIN, y);
  y += 5;

  doc.setFont("courier", "normal");
  doc.setFontSize(8);
  doc.setTextColor(65);
  doc.text(`Digital Hash (SHA-256): ${finalEvidenceHash}`, MARGIN, y);
  y += 5;
  doc.text(
    "System Integrity: NEXUS Sovereign Forensics Engine | Hash verified at time of generation.",
    MARGIN,
    y
  );
  y += 6;
...
export async function generateSection94Notice(
  ...
  const hash = evidenceHash ?? (await computeEvidenceHash({ trace, caseData }));
  const { doc, filename } = await buildSection94Notice(trace, caseData, hash);
  doc.save(filename);
}